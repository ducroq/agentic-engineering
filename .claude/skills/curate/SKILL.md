---
name: curate
description: End-of-session curation — freshness check, gotcha log review, promote patterns, update memory index
...
1. **Dead references**: Read `memory/MEMORY.md` and `CLAUDE.md`. For every file path mentioned, verify it still exists. List any broken paths.
...
3. **Lingering gotchas**: Read `memory/gotcha-log.md`. Flag any unresolved entries older than 14 days — they're either fixed (mark `[RESOLVED]`) or stuck (surface to the user).
4. **Ground truth drift**: For each ground truth file listed in the CLAUDE.md "Ground Truth Designations" table, verify the file exists and has been modified more recently than the artifacts that defer to it. Flag any where a downstream artifact is newer than its source of truth.
...
Skip if Step 0 already ran a full freshness check. Otherwise, spot-check that paths mentioned in the memory index and CLAUDE.md still exist. Flag any broken references.
